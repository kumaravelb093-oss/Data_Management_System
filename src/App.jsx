      ...
      <AnimatePresence>
        {notification && (
          <motion.div 
            ...
            exit={{ opacity: 0 }}
            className={`fixed bottom-8 right-8 px-6 py-4 rounded-xl shadow-2xl text-white font-bold text-sm z-[100] flex items-center gap-3 ${
...
const Dashboard = ({ records, loading, onRefresh, onViewRecord, onEditRecord }) => {
  ...
  const stats = {
    ...
    weekly: records.filter(r => new Date(r.entry_date_time) >= oneWeekAgo).length,
    monthly: records.filter(r => new Date(r.entry_date_time) >= oneMonthAgo).length,
  ...
  const filtered = records.filter(r => {
    ...
    const matchesTab = activeTab === 'All' || r.service_type === activeTab;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      ...
      {/* Timeline Analytics Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatItem label="Admission Today" value={stats.today} icon={<Clock size={20} />} color="text-amber-600" />
        <StatItem label="Recent (7 Days)" value={stats.weekly} icon={<TrendingUp size={20} />} color="text-blue-600" />
        <StatItem label="Monthly Volume" value={stats.monthly} icon={<Activity size={20} />} color="text-indigo-600" />
      ...
      <div className="relative group max-w-2xl">
        ...
        <input 
          type="text" 
          placeholder={`Search ${activeTab === 'All' ? 'all' : activeTab} assessments by name, mobile, or ID...`}
          ...
          <table className="w-full patient-table border-collapse">
            ...
            <tbody>
                ...
                <tr key={i} onClick={() => onViewRecord(record)} className="table-row">
                  ...
                  <td>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${
                      record.service_type === 'IP' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-blue-700'
                    }`}>
              ...
              {filtered.length === 0 && !loading && (
                <tr>
                  <td colSpan="6" className="py-20 text-center text-slate-400 font-medium">No patient assessments matching your filter.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                record.service_type === 'IP' ? 'bg-indigo-500 text-white' : 'bg-blue-500 text-white'
